...
Simulando os dados para um modelo $Y_t = \beta_0 + \beta_1Y_{t-1}^4 + \varepsilon_t$:
```{r}
set.seed(205650)
n = 500
b0 = 12
...
hist(y, main = "Histograma de Yt simulado")

data = matrix(c(y,x), byrow=F, ncol=2)
plot(x,y, main = "Gráfico de Yt")
```
...
library(PLRModels)
hcv = np.cv(dados)
hcvvo = hcv$h.opt$matrix.0..2..num.ln.[2]
hplug = dpill(x,y)
hplug

m1 = locpoly(x,y, bandwidth = hplug)
plot(x,y)
lines(m1$x, m1$y, col = "purple")

m1 = locpoly(x,y, bandwidth = hcvvo)
plot(x,y)
lines(m1$x, m1$y, col = "pink")

m1 = locpoly(x,y, bandwidth = hplug, degree = 2)
lines(m1$x, m1$y, col = "yellow")
```
