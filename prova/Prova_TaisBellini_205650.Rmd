...
Para esta questão, vamos selecionar 4 variáveis do banco de dados: Gold, Dollar Index, AMZN e Close. Vamos identificar se as séries possuem relação de cointegração para determinar se devemos utilizar um VEC. Para i
...
Nota-se que apenas a variável do Index do Dólar é estacionária. 

Agora, precisamos verificar a existência de cointegração entre as variáveis. Faremos isto através do método dos autovalores de Johansen, que permitem o teste para mais do que duas variáveis ao mesmo tempo. Para isto, temos o modelo VEC, que faz a diferenciação das séries obtendo a seguinte forma:
