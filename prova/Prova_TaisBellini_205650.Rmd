...
Nota-se que a variável DTB4WK não é estacionária. 
...
O lag ótimo obtivo para o VAR é 6, portanto, para o VEC, utilizamos 5:

```{r}
# K = lag -1
ctest1e <- ca.jo(ds, type = "eigen", ecdet = "const", K = 5)
summary(ctest1e)
```
...
```{r, warning = F}
vec_model <- VECM(ds, 5, r = 1, estim =("2OLS"))
summary(vec_model)
```

Observa-se pelo vetor de cointegração que o preço final do índice S&P tem uma relação positiva pequena com o índice do Dólar e um relação negativa maior com os títulos do Tesouro dos Estados Unidos. Ainda, percebe-se que o efeito do preço final do índice com defasagem 1, 3 e 5 é significativo para o preço final do índice. Os valores das duas demais variáveis têm uma relação de efeito significativa com os seus próprios lags. 
...
Vamos determinar a variável resposta e os preditores. A regress'ao ridge assume que a variável resposta está centralizada e os preditores padronizados:
```{r}
# variavel resposta:
...
Observa-se que os métodos com penalização resultam em mais resíduos, já que aplicam restrições a escolha do modelo. Porém a diferença é pequena e, pelo gráfico, observamos um ajuste parecido em todos. 
...
Observa-se que o método de cross-validation escolheu o $\lambda = 0.36$ e, em comparação com o $\lambda = 0.1$ zerou mais coeficientes, o que é esperado, pois quanto maior o $\lambda$, mais variáveis serão zeradas, como indicado no gráfico abaixo. 
...
O preço final do índice S&P nos lags 1 e 2 foram os mais significativos para determinar o preço final. As variáveis NYSE e DJI, que representam outros índices da bolsa de valores também se mostraram significativos. Ainda, grandes empresas como Amazon, Apple e o banco JP Morgan (incluindo alguns de seus lags) possuem influência no preço final. Com o $lambda = 0.35$, a maioria das variáveis com defasagem = 3 foram desconsideradas. Com o $\lambda = 0.1$,também observa-se a remoção de algumas variáveis com 3 defasagens removidas. No segundo cenário, 6 variáveis foram removidas do modelo, enquanto no primeiro 18 foram desconsideradas do modelo final. 
