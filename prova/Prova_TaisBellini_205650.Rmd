...
$$
Ainda, sabendo que uma FDP conjunta qualquer $f(X,Y)$ pode ser decomposta em $f(Y|X)f(X)$ podemos reescrever \ref{fmv} como:
$$
...
\label{ar1}
\end{equation}
com $\epsilon_t \sim i.i.d\ N(0, \sigma^2)$, a decomposição da função de máxima verossimilhança é feita da seguinte forma:

$$
L(\theta|\mathbf{Y}) = f(Y_t|Y_{t-1};\theta)f(Y_1;\theta)
$$
Conhecendo a distribuicão de $Y_t|\Im_{t-1}$, e sabendo que $\EX{Y_t} = \mu = \frac{\phi_0}{1-\phi_1}$, ou seja, $\phi_0 = \mu(1-\phi_1)$, então podemos escrever:
$$
f(Y_t|\Im_{t-1};\theta) \sim N(\mu(1-\phi_1) + \phi_1Y_{t-1},\sigma^2)
$$
$$
\sim N(\mu -\phi_1(Y_{t-1} - \mu),\sigma^2)
$$
Reescrevendo $\epsilon_t$ em \ref{ar1}, temos:
$$
\epsilon_t = Y_t - \phi_0 - \phi_1Y_{t-1} \\ 
= Y_t - \mu(1-\phi_1) - \phi_1Y_{t-1} \\
= (Y_t - \mu) - \phi_1(Y_{t-1} - \mu)
$$
Logo, dizemos que:
$$
f(Y_t|\Im_{t-1};\theta) = f_{\epsilon}\left[(Y_t - \mu) - \phi_1(Y_{t-1} - \mu)\right]
$$
onde $f_{\epsilon}$ é a densidade de $\epsilon_t$, portanto:
\begin{equation}
L(\theta|\mathbf{Y}) = f_{\epsilon}\left[(Y_t - \mu) - \phi_1(Y_{t-1} - \mu)\right]f(Y_1;\theta)
\label{fmv_mod}
\end{equation}

Para $f(Y_1;\theta)$, vamos considerar que, com $|\phi_1| < 1$ o modelo AR(1):
$$
\tilde{Y_t} = \phi_1\tilde{Y}_{t-1} + \epsilon_t
$$
com $\tilde{Y_t} = Y_t - \mu$ e $\epsilon_t \sim i.i.d\ N(0, \sigma^2)$ independentes, $\tilde{Y_t}$ pode ser definido, de acordo com @Morettin2006, por:
$$
\tilde{Y_t} = \sum_{j=0}^{\infty}\phi_1^j\epsilon_{t-j}
$$
onde $\tilde{Y_t} \sim N(0, \sigma^2(1-\phi_1^2)^{-1})$. Portanto, vamos ter que:
$$
\tilde{Y_1} = \sum_{j=0}^{\infty}\phi_1^j\epsilon_{t-j} = v
$$
$$
\tilde{Y_t} = \phi_1\tilde{Y}_{t-1} + \epsilon_t\ t = 2, \dots, n
$$
em que:
\begin{equation}
(v, \epsilon_2, \dots, \epsilon_n) \sim N(0,\Sigma) 
\label{fmv_mod_dist}
\end{equation}
com $\Sigma = diag((1-\phi_1^2)^{-1}), \sigma^2, \dots, \sigma^2)$.

Assim, determinamos a densidade conjunta:
$$
f(v,  \epsilon_2, \dots, \epsilon_n) = \left[\frac{(1-\phi_1^2)}{2\pi\sigma^2}\right]^{1/2}exp\left[\frac{-\tilde{Y_1}(1-\phi_1^2)}{2\sigma^2}\right]\left[\frac{1}{2\pi\sigma^2}\right]^{\frac{n-1}{2}} exp\left[\frac{-1}{2\sigma^2}\sum_{t=2}^n(\tilde{Y_t} - \phi_1\tilde{Y}_{t-1})^2\right]
$$
e o log da verossimilhança é:
$$
l(\theta|\mathbf{Y}) = -\frac{n}{2}ln(2\pi) + \frac{1}{2}ln(1-\phi_1^2) - \frac{n}{2}ln(\sigma^2) - \frac{S(\eta)}{2\sigma^2}
$$
$$
S(\eta) = (Y_1-\mu)^2(1-\phi_1^2) + \sum_{t=2}^n[(Y_t-\mu) -\phi_1(Y_{t-1}-\mu)]^2
$$
que deve ser derivada e igualada a zero para definir os valores de $\theta$ estimados.

Por \ref{fmv_mod} e \ref{fmv_mod_dist}:
$$
\hat{\phi_1}_{EMV} \sim N(0, \frac{1-\phi_1}{n})
$$
$$
\sqrt{n}(\hat{\phi_1}_{EMV} - \phi_1) \xrightarrow{D} N(0, (1-\phi_1^2))
$$
...
\begin{equation}
\begin{aligned}
y_{jt} = c_j & + \phi_{j1}^{(1)}y_{1,t-1} + \dots + \phi_{jn}^{(1)}y_{n,t-1} + \dots
...
Portanto, o VAR(p) é um sistema em que cada variável é regredida em uma constante e p de seus lags, assim como p lags de cada uma das demais variáveis.
...
O estimador de máxima verossimilhança deste sistema, como visto em aula, é definido da seguinte forma:

\begin{equation}
L(\mathbf{\theta}) = \prod_{t=1}^{T}f(\mathbf{y_t}|\Im_{t-1};\theta) = (2\pi)^{-n/2}|\Omega^{-1}|^{1/2}exp\{-\frac{1}{2}\sum_{t=1}^T(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})^\intercal\Omega^{-1}(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})\}
\label{eq:lvar}
\end{equation}
...
\begin{equation}
l(\mathbf{\theta}) = \sum_{t=1}^{T}logf(\mathbf{y_t}|\Im_{t-1};\theta) = (-Tn/2)log(2\pi)+(T/2)log|\Omega^{-1}|(-\frac{1}{2})\sum_{t=1}^T(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})^\intercal\Omega^{-1}(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})
\label{eq:logvar}
\end{equation}

Fazendo a derivação em relação a $\Pi$ de \ref{eq:logvar} para obter o estimador de MV dos parâmetros do modelo, obtemos o seguinte resultado, como demonstrado em @Hamilton1994:
$$
...
Para mostrar que \ref{eq:varmv} é equivalente ao estimador de mínimos quadrados ordinários (OLS) da linha j, vamos expandir o último termo de \ref{eq:logvar}, somando e subtraindo $\hat{\Pi}^\intercal \mathbf{x_t}$ e determinando $\hat{\epsilon_t} = y_t - \hat{\Pi}^\intercal \mathbf{x_t}$ como o vetor de resíduos amostrais da observação $t$ de uma regressão OLS de $y_{jt}$ em $x_t$, $j = 1, ..., p$:

\begin{equation}
\sum_{t=1}^T\left[(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})^\intercal\Omega^{-1}(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})\right]
\label{eq:minimize}
\end{equation}

$$
= \sum_{t=1}^T\left[(\mathbf{y_t}-\hat{\Pi}^\intercal \mathbf{x_t} + \hat{\Pi}^\intercal \mathbf{x_t} - \Pi^\intercal\mathbf{x_t})^\intercal\Omega^{-1}(\mathbf{y_t}-\hat{\Pi}^\intercal \mathbf{x_t} + \hat{\Pi}^\intercal \mathbf{x_t} - \Pi^\intercal\mathbf{x_t})\right]
$$

$$
= \sum_{t=1}^T\left[(\hat{\epsilon_t} + (\hat{\Pi} - \Pi)^\intercal \mathbf{x_t})^\intercal\Omega^{-1}(\hat{\epsilon_t} + (\hat{\Pi} - \Pi)^\intercal \mathbf{x_t})\right]
$$
Ainda, podemos seguir com:

\begin{equation}
= \sum_{t=1}^T\hat{\epsilon_t}^\intercal\Omega^{-1}\hat{\epsilon_t} + 2 \sum_{t=1}^T\hat{\epsilon_t}^\intercal\Omega^{-1}(\hat{\Pi} - \Pi)^\intercal \mathbf{x_t} + \sum_{t=1}^T \mathbf{x_t}^\intercal(\hat{\Pi} - \Pi)\Omega^{-1}(\hat{\Pi} - \Pi)^\intercal \mathbf{x_t}
\label{eq:varexp}
\end{equation}

O termo do meio da equação \ref{eq:varexp} é um escalar, portanto, podemos aplicar a operação de traço e obter o seguinte resultado:

$$
trace\left[\sum_{t=1}^T\hat{\epsilon_t}^\intercal\Omega^{-1}(\hat{\Pi} - \Pi)^\intercal \mathbf{x_t} \right] = trace\left[\Omega^{-1}(\hat{\Pi} - \Pi)^\intercal\sum_{t=1}^T\mathbf{x_t}\hat{\epsilon_t}^\intercal\right]
$$
 Por definição, os resíduos de uma regressão de mínimos quadrados ordinários são ortogonais às variáveis respostas, ou seja, $\sum_{t=1}^T\mathbf{x_t}\hat{\epsilon_t}^\intercal = 0$ e podemos zerar o termo do meio de \ref{eq:varexp}. Ainda, $\Omega^{-1}$ é uma matriz positiva definida. Assim, definindo $\mathbf{x_t}^{*} = (\hat{\Pi} - \Pi)^\intercal\mathbf{x_t}$ temos que \ref{eq:minimize} se dá por: 
 
$$
\sum_{t=1}^T\left[(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})^\intercal\Omega^{-1}(\mathbf{y_t}-\Pi^\intercal\mathbf{x_t})\right] = \sum_{t=1}^T\hat{\epsilon_t}^\intercal\Omega^{-1}\hat{\epsilon_t} + \sum_{t=1}^T[\mathbf{x_t}^{*}]^\intercal\Omega^{-1}\mathbf{x_t}^{*}
$$

Este resultado é positivo para qualquer sequência $\{\mathbf{x_t}^{*}\}_{j=1}^{T}$ menos quando  $\mathbf{x_t}^{*}=0$. Portanto, o que minimiza \ref{eq:minimize} é o $\mathbf{x_t}^{*}=0$, ou seja, $\hat{\Pi} = \Pi$. Logo, temos que \ref{eq:logvar} é maximizada quando $\hat{\Pi} = \Pi$ e temos que a regressão por mínimos quadrados ordinários provê o estimador de máxima verossimilhança dos coeficientes do VAR.
...
Nota-se que apenas a variável índice do dólar é estacionária. 
...
O teste dos autovalores de Johansen verifica sequencialmente se o posto da matriz $A$ é 0, 1, 2, ... até o número de séries sendo avaliadas menos um. Se o posto da matriz A = 0, não há co-integração.
...
O lag ótimo obtido para o VAR é 5, portanto, para o VEC, utilizamos 4:
...
Vamos determinar a variável resposta e os preditores. A regressão ridge assume que a variável resposta está centralizada e os preditores padronizados:
```{r}
# variavel resposta:
y = scale(df$close, center = T, scale = F)
...
names(B.hat.RR.005) = names(B.hat.RR.01) = names(B.hat.OLS)
...
colnames(sums) = c("Sum of coefs OLS", "Sum of coefs lambda = 0.05", 
                   "Sum of coefs lambda = 0.1")
kable(sums)
```
...
Observa-se que os métodos com penalização resultam em menos resíduos, indicando um ajuste melhor do modelo. Porém a diferença é pequena e, pelo gráfico, observamos um ajuste parecido em todos.
...
Testando com os valores de $\lambda$ fixado em 0.1 e escolhido por cross-validation:
```{r}
set.seed(205650)
lasso_cv <- cv.glmnet(X, y, alpha = 1, lambda = lambdas_to_try, standardize = F, nfolds = 10)
lambda_cv <- lasso_cv$lambda.min

fit.LASSO.CV = glmnet(y=y,x=X, lambda = lambda_cv, alpha = 1)
...
lambdas = cbind(lambda_cv, 0.1)
coefs = cbind(B.hat.LASSO.CV, B.hat.LASSO.Fixed01)
rownames(lambdas) = "Lambda value"
kable(rbind(coefs, lambdas))
```

Observa-se que o método de cross-validation escolheu o $\lambda = 0.03$ e, em comparação com o $\lambda = 0.1$ zerou menos coeficientes, o que é esperado, pois quanto maior o $\lambda$, mais variáveis serão zeradas, como indicado no gráfico abaixo. 
...
As variáveis NYSE e DJI, que representam outros índices da bolsa de valores, e seus lags se mostraram significativos para o preço final do índice S&P. Ainda, como na questão 3, observa-se um efeito tardio do preço final do índice, com um alto coeficiente para o lag 3 da variável Close. Com o $lambda = 0.1$, a maioria das variáveis com defasagem = 3 foram desconsideradas. Com o $\lambda = 0.03$, apenas o valor da empresa americana Apple com 3 defasagens, o valor do ouro e o valor do banco JP Morgan com zero e 1 defasagens foram removidos do modelo.
...
O modelo com o $\lambda = 0.03$ obteve um melhor ajuste na predição, já que utiliza mais variáveis. Porém, observamos no gráfico que ambos fazem um bom ajuste.
...
Implementando AdaLasso com $\lambda$ escolhido por cross-validation e $\gamma = 0.5$:
```{r}
...
fit.adaLASSO = glmnet(y=y,x=X, lambda = lambda_cv, alpha = 1, penalty.factor = w)
...
Observa-se que com este método as variáveis que indicam o valor do petróleo com zero e 3 defasagens foram removidas e o valor da empresa Apple foi adicionado, em comparação com o método de regressão LASSO.
...
Para este caso, a regressão LASSO obteve um número levemente menor de resíduos, mas a diferença de ajuste entre os dois foi pequena e ambos fizeram um bom ajuste.
...
```{r, warning=F, message=F}
library(vip)
vip(sp.close, num_features = 15, bar = FALSE)
```
...
```
Notamos que a partir de 9 splits, começamos a ter uma estabilidade nos erros de predição. Portanto, vamos tentar fazer a poda da árvore para apenas 10 splits:
...
Perecebe-se um erro médio maior, em comparação com a árvore sem poda, o que é esperado. Porém o ajuste ficou muito longe do ideal e apenas as variáveis de defasagem do preço foram utilizadas. Vamos tentar uma abordagem intermediária, com menos splits que a árvore original, mas mais do que apenas 10:
...
```
Com 50 splits temos uma árvore maior, que utiliza mais algumas variáveis para a tomada de decisão, como o preço dos índices NYSE e DJI. O erro médio ainda é maior do que da árvore original, mas menos que a metade do que com 10 splits. 
...
```
Percebe-se ainda que o preço final do índice S&P com 1, 2 e 3 defasagens tem bastante importância para a árvore de decisão. Em seguida, os valores do petróleo, do Euro e os índices NYSE e DJI. Nota-se uma diferença na importância das variáveis em comparação com o método anterior.
...
plot(x,y, main = "Yt grau=1 e h = 1", xlab="Y_t-1")
lines(m1$x, m1$y, col = "red")
m2 = locpoly(x, y, bandwidth = 100)
plot(x,y, main = "Yt grau=1 e h = 100", xlab="Y_t-1")
lines(m2$x, m2$y, col = "blue")
m3 = lm(y ~ x)
plot(x,y, main = "Yt regressão linear", xlab="Y_t-1")
abline(m3, col = "orange")
m4 = locpoly(x, y, bandwidth = 0.1)
plot(x,y, main ="Yt com degree=1 e h = 0.1", xlab="Y_t-1")
lines(m4$x, m4$y, col = "red")

```

Observa-se que, mesmo utilizando um polinômio de Taylor de grau 1, obtemos uma boa estimação da função $Y_t$, mesmo ela não sendo linear. Note que, com o $h$ muito grande, não foi possível fazer um ajuste adequado, já que praticamente não foi feita a estimação local. Ainda, com $h=100$ e com uma regressão linear obtemos o mesmo resultado, pois a função Kernel não exerce influência na escolha de pontos vizinhos de $y$. Por fim, o $h$ muito pequeno começa a demonstrar _overfitting_.
...
plot(x,y, main = "Yt com h por CV", xlab="Y_t-1")
lines(m1$x, m1$y, col = "purple")

m2 = locpoly(x,y, bandwidth = hplug)
plot(x,y, main = "Yt com h por plug-in", xlab="Y_t-1")
