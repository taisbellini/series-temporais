...
abstract: |
  ...
  In this work, we propose a method for global variable selection and coefficient estimation, similarly to the approach presented in @Sottile2020. We introduce group (ada)LASSO penalty, suggested in @Yuan2006, and apply it to Quantile Autoregressive Distributed Lag (QADL) model. Furthermore, since we are in a time series context, we also evaluate the variable selection penalization applying higher penalties to higher lags, as introduced by @Konzen2016. The results suggest that a weighted, lag-penalized approach can provide better results in selecting covariates in models with up to five relevant lags of the regressors, without compromising the estimation of the coefficients. In particular, both LASSO and group LASSO penalization with higher weights for higher lags removed the non-relevant variables more often than the competitors in these scenarios.
keyword-subclass: | 
