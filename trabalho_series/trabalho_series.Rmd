...
Quantile regression, proposed by @Koenker1978, provides an estimation of the conditional distribution of a variable $Y$ on $\mathbf{X}$ at different quantile levels, offering a more robust estimation for outliers, as opposed to classical regression. (need a ref) @Galvao2013 proposes a study of quantile regression in an time series context, using a quantile autoregressive distributed lag model. (provavelmente a ref original de QADL)


This work aims to evaluate a proposed estimator in a Quantile Autoregressive Distributed Lag Model (QADL) context with variable selection using a regularization technique. 
...
$$y_t = c + \alpha_1y_{t-1} +  \cdots + \alpha_py_{t-p} + \theta_0\mathbf{x}_t^\intercal + \theta_1\mathbf{x}_{t-1}^\intercal + \cdots + \theta_q\mathbf{x}_{t-q}^\intercal + \varepsilon_t$$ 
where $t = 1, ..., n$, $y_t$ is the response variable, $y_{t-j}$ the lag of the response variable and $\mathbf{x}_t$ is the covariates vetor with dimension $d$. $\varepsilon_t$ is a white noise.

Now, to define a Quantile Autoregressive Distributed Lag Model (QADL), lets define ${U_t}$ as a sequence of i.i.d standard uniform random variables, $c$, $\alpha$ and $\theta$ as unknown functions $[0,1] \rightarrow \mathbb{R}$ to be estimated.

Then, we say that the $\tau_{th}$ conditional quantile function of $y_t$ is

$$
Q_{y_t}(\tau|\Im_t) =  c(\tau) + \alpha_1(\tau)y_{t-1} + \cdots + \alpha_p(\tau)y_{t-p} + \theta_0(\tau)\mathbf{x}_t^\intercal + \theta_1(\tau)\mathbf{x}_{t-1}^\intercal + \dots + \theta_q(\tau)\mathbf{x}_{t-q}^\intercal
$$


Let $(y_t, \mathbf{x}_t)$ be a sequence where $y_t$ is a scalar random variable for each $t$ and $\mathbf{x}_t$ a random vector of dimension $d$. 
