...
$\Im_t$ is the $\sigma$-field generated by ${y_s, s\leq t}$. 


## Global coefficient estimation and variable selection

Let $\mathbf{z_t}^\intercal = (1, y_{t-1}, \cdots, y_{t-p}, \mathbf{x}_{t-1}, \cdots, \mathbf{x}_{t-p})$ and
$\beta(\tau)^\intercal = (c(\tau), \alpha_1(\tau), \cdots, \alpha_p(\tau), \theta_1(\tau), \cdots , \theta_q(\tau))$. Then, equation \ref{eq:qadl} can be expressed in the form of 
\begin{equation}
