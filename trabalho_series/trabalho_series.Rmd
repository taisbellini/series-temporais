...
Sobre notação: embora na lit. de time series haja um costume de se denotar as variáveis aleatórias por letras minúsculas, e embora eu ache essa notação mais bonita, esse é um daqueles casos em que o senso estético deve ser sacrificado para se atingir uma notação mais clara e rigorosa. Sugiro fortemente que tu utilizes letras maiúsculas para denotar todas as variáveis e vetores envolvidos: Y_t, X_t, etc. Se tu achares mais claro fazer a distinção, podes usar um \boldsymbol{X} para enfatizar que as covariáveis são vetores, mas isso não é estritamente necessário, desde que esteja explicitado no texto.
--->
...
$$
where $j = 1, \dots,q$ and $q$ is the number of covariates of the model. $\varphi$ will have dimensions $q\times(k+1)$, so each covariate has $k+1$ associated parameters. 
...
$\varphi(\tau)$ is a $l \times M$ matrix, the resulting dimensions of $\beta(\tau|\varphi)$ is $d \times M$, being $M$ the size of the quantile grid we are estimating. Thus, we have that:
...
Since $\varphi(\tau)$ is known, our goal is to find $\delta$ that minimizes the loss function of the standard QR:

$$
\hat{\beta(\tau|\varphi)} = \arg\min_{\delta} \sum_{i=1}^n{\rho_\tau(y_i - \mathbf{z_i}^\intercal \delta\varphi(\tau))} = \arg\min_{\delta} \sum_{i=1}^n{(\tau - \mathbb{I_{[y_i\leq\mathbf{z_i}^\intercal\delta\varphi(\tau)]}})(y_i - \mathbf{z_i}^\intercal \delta\varphi(\tau))}
$$
