...
In a time series context, we are estimating the coefficients for each lagged variable, thus, (Park and Sakaori 2013) consider applying higher weights for higher lags, as it is usual that more recent variables are more relevant. @Konzen2016 apply this idea to propose the WLadaLASSO, that has its original form described as $\omega_j = \left(|\hat{\beta_j}|e^{-\alpha t}\right)^{-p}$, with $p>0$, $\alpha \geq 0$ and $t$ representing the lag order. For \ref{eq:betafuncpenAda}, $\omega_j = \left[\left(\sum_{h=1}^L|\hat{\delta_{jh}}|\right)e^{-\alpha t}\right]^{-p}$.
...
```{r Ycorr, echo=FALSE, warning=F,fig.cap="\\label{fig:Ycorr}ACF,PACF and correlations of Yt and Xt"}
...
```{r Yfigs, echo=FALSE, warning=F, fig.width=7,fig.height=6,fig.cap="\\label{fig:Y}Y time series"}
par(mfrow=c(1,2))
hist(Y, border=NA, breaks="FD")
ts.plot(Y)
```
Figure \ref{fig:Ycorr} illustrate the autocorrelation function and the partial autocorrelation function of the process generated by the method described, the correlation between $Yt$ and $Y_{t-1}$ as well as the correlation between $Yt$ and $X_{t-1}$.
