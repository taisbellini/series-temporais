...
To test the proposed approach, we used Monte Carlo simulations with 500 replications using the method described in section \ref{simulation} to generate QADL processes of 500 observations. We generate three different processes: the first, as described in \ref{qadl_gen}; the second, replacing $Y_{t-1}$ and $X_{t-1}$ by $Y_{t-2}$ and $X_{t-2}$,respectively; the last, replacing $Y_{t-1}$ by $Y_{t-5}$.
...
```{r Ycorr3, echo=FALSE, warning=F, fig.width=7,fig.height=6, fig.pos = 'H', fig.cap="\\label{fig:Ycorr3}ACF,PACF and correlations of Yt and Xt"}
...
We observed two aspects: the coefficients estimation and the variable selection. The metric used to evaluate the estimation of the relevant variables was Mean Squared Error (MSE) by coefficient . The number of times the non-relevant covariates were set to zero was used to analyze the effectivity in selecting variables.
...
```{r mse_Y1X1_table, echo=FALSE, warning=F, out.width="70%", fig.pos = 'H', fig.cap="\\label{fig:Y1X1_mse_table} MSE table of coefficient estimation"}
...
```{r vs_Y5X1, echo=FALSE, warning=F, out.width="90%", fig.pos = 'H', fig.cap="\\label{fig:Y5X1_vs} Number of times each variable was set to zero"}
...
```{r mse_Y5X1_table, echo=FALSE, warning=F, out.width="70%", fig.pos = 'H', fig.cap="\\label{fig:Y5X1_mse_table} MSE table of coefficient estimation"}
...
-> piqr and gLasso didn`t select variables, but need to consider lambda. Further investigation should find the optimal lambda for each method.
